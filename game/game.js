...
(async function () {
	...
	const globalText = {
		...
		SERVICE_WORKER_NOT_SUPPORT: ["您使用的客户端或浏览器不支持启用serviceWorker", "游戏将在降级模式下运行，部分功能可能受限！"].join("\n"),
	...
	// 检查serviceWorker支持，但不强制要求
	const serviceWorkerSupported = "serviceWorker" in navigator;
	if (!serviceWorkerSupported) {
		console.warn("ServiceWorker不支持，将使用降级模式运行游戏");
	...
	if (serviceWorkerSupported) {
		...
		try {
			...
			console.warn("serviceWorker加载失败: 部分功能可能受限");
		}
	}
